...
export const CELION_A5_SLIDE_HTML_SPEC = [
  ...
  `Each .slide is fixed at ${SLIDE_SIZE_PX.width}px x ${SLIDE_SIZE_PX.height}px or ${SLIDE_SIZE_MM.width}mm x ${SLIDE_SIZE_MM.height}mm`,
  "Each .slide uses overflow: hidden and page-break-after/break-after",
  `@page is ${SLIDE_SIZE_CSS_MM} with zero margin`,
...
export function validateCelionSlideHtml(
  ...
  const pageSizePattern = new RegExp(`@page\\s*\\{[^}]*${SLIDE_SIZE_MM.width}mm\\s+${SLIDE_SIZE_MM.height}mm[^}]*margin\\s*:\\s*0`, "i");
  const fixedWidthPattern = new RegExp(`width\\s*:\\s*(${SLIDE_SIZE_PX.width}px|${SLIDE_SIZE_MM.width}mm)`, "i");
  const fixedHeightPattern = new RegExp(`height\\s*:\\s*(${SLIDE_SIZE_PX.height}px|${SLIDE_SIZE_MM.height}mm)`, "i");

  if (!pageSizePattern.test(html)) {
    errors.push(`Output must include @page { size: ${SLIDE_SIZE_CSS_MM}; margin: 0; }.`);
  }
